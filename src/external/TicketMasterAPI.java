...
public class TicketMasterAPI implements ExternalAPI {
	...
	private static final String API_KEY = "p9IUYoVcRKB35cR2UricU0l5hAheHR9j";
	...
	private List<Item> getItemList(JSONArray events) throws JSONException {
		...
		for (int i = 0; i < events.length(); i++) {
			...
			builder
				.setItemId(getStringFieldOrNull(event, "id"))
				.setName(getStringFieldOrNull(event, "name"))
				.setDescription(getDescription(event))
				.setCategories(getCategories(event))
				.setImageUrl(getImageUrl(event))
				.setUrl(getStringFieldOrNull(event, "url"));
